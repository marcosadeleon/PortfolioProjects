...
SELECT YEAR(CAST(str_to_date(date, '%m/%d/%Y') AS date)) as year,
...
WHERE location LIKE '%states%'
GROUP BY year, month
ORDER BY PercentPopulationInfected_ThisMonth DESC;
...
AND coviddeaths.location = vac.location
...
SELECT YEAR(CAST(str_to_date(coviddeaths.date, '%m/%d/%Y') AS date)) as year,
...
WITH PopVaccinated(Year, Month, population, TotalPeopleVaccinated_Cumulative)
AS
(
SELECT YEAR(CAST(str_to_date(coviddeaths.date, '%m/%d/%Y') AS date)) as year,
       MONTH(CAST(str_to_date(coviddeaths.date, '%m/%d/%Y') AS date)) as month,
...
INSERT INTO UnitedStatesVaccinationbyMonth
SELECT YEAR(CAST(str_to_date(coviddeaths.date, '%m/%d/%Y') AS date)) as year,
       MONTH(CAST(str_to_date(coviddeaths.date, '%m/%d/%Y') AS date)) as month,
       coviddeaths.population,
       MAX(covidvaccinations.people_vaccinated) as TotalPeopleVaccinated_Cumulative
...
ON coviddeaths.date = covidvaccinations.date
AND coviddeaths.location = covidvaccinations.location
WHERE (covidvaccinations.people_vaccinated/coviddeaths.population)*100 > 50
GROUP BY coviddeaths.location
ORDER BY year, month;
